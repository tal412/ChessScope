          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2">
              <div className="bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-500/30 rounded-lg p-4">
                <h3 className="text-blue-400 font-medium mb-2">📊 Smart Analysis</h3>
                <p className="text-slate-300 text-sm mb-2">
                  Build a unified graph of chess positions from your games for detailed opening analysis.
                </p>
                <ul className="text-slate-400 text-xs space-y-1">
                  <li>• Each position analyzed with game statistics</li>
                  <li>• Efficient storage and fast lookups</li>
                  <li>• Complete opening tree visualization</li>
                  <li>• Win rate tracking per position</li>
                </ul>
              </div>
            </div>

            {/* Stats and Storage Info */}
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 text-center">
                <div className="bg-slate-700/30 p-3 rounded-lg">
                  <div className="text-lg font-bold text-green-400">{storageInfo.totalGraphs}</div>
                  <div className="text-xs text-slate-400">Users</div>
                </div>
                <div className="bg-slate-700/30 p-3 rounded-lg">
                  <div className="text-lg font-bold text-blue-400">{storageInfo.totalSizeMB} MB</div>
                  <div className="text-xs text-slate-400">Storage Used</div>
                </div>
              </div>

              {storageInfo.graphs.length > 0 && (
                <div className="bg-slate-700/20 rounded-lg p-3">
                  <h4 className="text-slate-300 text-sm font-medium mb-2">Imported Data:</h4>
                  <div className="space-y-2 max-h-32 overflow-y-auto">
                    {storageInfo.graphs.map((graph, index) => (
                      <div key={index} className="flex justify-between items-center text-xs">
                        <div className="flex items-center gap-2">
                          <span className="text-slate-400">{graph.username}</span>
                          <span className="text-slate-500">
                            W: {graph.stats?.white?.totalGames || 0} | 
                            B: {graph.stats?.black?.totalGames || 0}
                          </span>
                        </div>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => {
                            localStorage.setItem('chesscope_username', graph.username);
                            setUsername(graph.username);
                            setSuccess(`Active user set to: ${graph.username}`);
                          }}
                          className="bg-slate-600/50 border-slate-500 text-slate-300 hover:bg-slate-500/50 text-xs px-2 py-1 h-6"
                        >
                          Use
                        </Button>
                      </div>
